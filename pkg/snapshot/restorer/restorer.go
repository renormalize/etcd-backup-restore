...
func (r *Restorer) restoreFromBaseSnapshot(ro brtypes.RestoreOptions) (err error) {
	...
	if err != nil {
		return fmt.Errorf("failed to decode the snapshot compression policy with error: %w", err)
	}
	if isCompressed {
	...
	db, err := os.CreateTemp("", "snapshot-*.db")
	if err != nil {
	...
	// Ensure the directory(ies) with the old, possibly empty/corrupt data has been removed
	// The Restore API call will fail if the data directory or the wal directory exist
	err = os.RemoveAll(ro.Config.DataDir)
	if err != nil {
		return fmt.Errorf("unable to remove the old data directory during restoration with error: %w", err)
	}
